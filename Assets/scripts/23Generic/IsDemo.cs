...
public class IsDemo : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (j is int)
        {
            Debug.Log($"[2] {j}는 null이 아니며 정수형 형식으로 변환이 가능하다");
        }
    }
...
박싱(Boxing)      : 값 형식의 데이터를 참조 형식의 데이터로 변경하는 것 ( int > object )
언박싱(UnBoxing)  : 참조 형식의 데이터를 값 형식의 데이터로 변경하는 것 - 캐스트,캐스팅 ( object > int )
